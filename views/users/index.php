...
<section class="panel">
    <div class="panel-header"><h2>Pending Access Requests</h2></div>
    <div class="table-wrap">
        <table>
            <thead>
            <tr>
                <th>Name</th><th>Email</th><th>Requested Role</th><th>Message</th><th>Requested At</th><th>Actions</th>
            </tr>
            </thead>
            <tbody>
            <?php if (!$pendingAccessRequests): ?>
                <tr><td colspan="6">No pending requests.</td></tr>
            <?php endif; ?>
            <?php foreach ($pendingAccessRequests as $request): ?>
                <tr>
                    <td><?= e($request['full_name']); ?></td>
                    <td><?= e($request['email']); ?></td>
                    <td><?= e($request['desired_role']); ?></td>
                    <td><?= e((string) $request['message']); ?></td>
                    <td><?= e($request['created_at']); ?></td>
                    <td class="action-group request-actions">
                        <form class="request-action-form" method="post" action="<?= e(basePath('index.php?page=users')); ?>">
                            <input type="hidden" name="csrf_token" value="<?= e(csrfToken()); ?>">
                            <input type="hidden" name="action" value="approve_access_request">
                            <input type="hidden" name="request_id" value="<?= e((string) $request['id']); ?>">
                            <input class="action-note" type="text" name="review_note" placeholder="Approval note">
                            <button class="button small primary" type="submit">Approve</button>
                        </form>
                        <form class="request-action-form" method="post" action="<?= e(basePath('index.php?page=users')); ?>">
                            <input type="hidden" name="csrf_token" value="<?= e(csrfToken()); ?>">
                            <input type="hidden" name="action" value="reject_access_request">
                            <input type="hidden" name="request_id" value="<?= e((string) $request['id']); ?>">
                            <input class="action-note" type="text" name="review_note" placeholder="Rejection reason">
                            <button class="button small danger-outline" type="submit">Reject</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
